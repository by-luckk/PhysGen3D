...
class MPMSolver:
    ...
    def __init__(
        ...
        self.mu_0, self.lambda_0 = self.E_0 / (
            2 * (1 + self.nu_0)), self.E_0* self.nu_0 / ((1 + self.nu_0) *
                                                    (1 - 2 * self.nu_0))
        ...
        if self.quant:
            ...
            if self.use_emitter_id:
                self.particle.place(self.x, self.v, self.F, self.material,
                                self.color, self.rho, self.emitter_ids)
            else:
                self.particle.place(self.x, self.v, self.F, self.material,
                                self.color, self.rho)
            if self.support_plasticity:
    ...
    def g2p2g(self, dt: ti.f32, pid: ti.template(), grid_v_in: ti.template(),
        ...
        for I in ti.grouped(pid):
            ...
            if ti.static(self.use_diff_E):
                ...
                la = self.E[p] * self.nu[p] / ((1 + self.nu[p]) * (1 - 2 * self.nu[p])) * h
            else:
            ...
            for offset in ti.static(ti.grouped(self.stencil_range())):
                ...
                grid_v_out[base +
                           offset] += weight * (self.rho[p] * self.p_vol * self.v[p] +
                                                affine @ dpos)
    ...
    @ti.func
    def seed_particle_colors(self, i, x, material, color, velocity, emmiter_id):
        self.x[i] = x
        self.v[i] = velocity
        self.F[i] = ti.Matrix.identity(ti.f32, self.dim)
        self.color[i] = color
    ...
    @ti.kernel
    def seed(self, new_particles: ti.i32, new_material: ti.i32, color: ti.i32, E: ti.f32, nu: ti.f32, rho: ti.f32):
    ...
    def seed_ellipsoid(self, new_particles: ti.i32, new_material: ti.i32,
                       color: ti.i32, E: ti.f32, nu: ti.f32, rho: ti.f32):
    ...
    def seed_from_external_array(self, num_particles: ti.i32,
                                 ...
                                 color: ti.i32, E: ti.f32, nu: ti.f32,
                                 rho: ti.f32):
        ...
        for i in range(num_particles):
            ...
            if ti.static(self.use_diff_E):
                ...
                self.nu[p] = nu
            self.rho[p] = rho
            self.seed_particle(p, x, new_material,
                               color, self.source_velocity[None], None)
        self.n_particles[None] += num_particles
                               
    @ti.kernel
    def seed_from_external_array_colors(self, num_particles: ti.i32,
                                 pos: ti.types.ndarray(), new_material: ti.i32,
                                 colors: ti.types.ndarray(), E: ti.f32, nu: ti.f32,
                                 rho: ti.f32):
        ...
        for i in range(num_particles):
            ...
            if ti.static(self.use_diff_E):
                ...
                self.nu[p] = nu
            self.rho[p] = rho
            self.seed_particle_colors(p, x, new_material,
                               colors[i], self.source_velocity[None], None)
    ...
    def add_particles(self,
                      ...
                      material,
                      color=0xFFFFFF,
                      velocity=None, E=1e6, nu=0.2, rho=1000):
        self.set_source_velocity(velocity=velocity)
        self.seed_from_external_array(len(particles), particles, material,
                                      color, E, nu, rho)
        
    def add_particles_color(self,
                      particles,
                      colors,
                      material,
                      velocity=None, E=1e6, nu=0.2, rho=1000):
        self.set_source_velocity(velocity=velocity)
        self.seed_from_external_array_colors(len(particles), particles, material,
                                      colors, E, nu, rho)
